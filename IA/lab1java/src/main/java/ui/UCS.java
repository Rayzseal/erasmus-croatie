...
/**
 * 
 * This class is used to compute the algorithm UCS used to find the shortest path from 
 * an initial node to one (there could be multiple) goal states. 
 * 
 * Source used (pseudo code of the algorithm)
 * https://ai.stackexchange.com/questions/17904/understanding-the-pseudocode-of-uniform-cost-search-from-the-book-artificial-in
 * 
 * @author chloe
 *
 */
public class UCS {
	...
	/**
	 * Compute the UCS algorithm. 
	 * @param graph graph to be used. 
	 */
	static void UCS(Graph graph) {
		...
		do {
			...
			// Check if the starting node is already in the queue of sorted nodes 
			...
			for (Transitions adj : currend.getTransitions()) {
				...
				if (!sortedset.contains(child) && !explored.contains(child)) {
					...
					child.setCost(currend.getCost() + adj.getWeight());
					// Parent node is set
		...
		// If no result has been found, display the results
		print(found,explored.size());
	}
